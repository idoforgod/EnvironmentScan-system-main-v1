...
### Step 2.1: Signal Classification, Impact Analysis (Unified)
...
- **pSST**: ES + CC (classification) → IC (impact), all in-context
- **Note**: priority-ranked is NOT produced here — Python Step 2.3 handles it

### Step 2.2: Impact Analysis Output Verification
- **Worker**: (none — @phase2-analyst produced this in Step 2.1)
- **Input**: `{data_root}/structured/classified-signals-{date}.json` (verify exists)
- **Output**: `{data_root}/analysis/impact-assessment-{date}.json` (verify exists)
