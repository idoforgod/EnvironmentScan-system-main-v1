...
| `TIMELINE_MAP_ENABLED` | `system.signal_evolution.timeline_map.enabled` | `true` |
| `TIMELINE_SCRIPT` | `system.signal_evolution.timeline_map.generator_script` | `env-scanning/core/timeline_map_generator.py` |
| `TIMELINE_ORCHESTRATOR` | `system.signal_evolution.timeline_map.orchestrator` | `.claude/agents/timeline-map-orchestrator.md` |
| `TIMELINE_SUMMARY_FILE` | (derived) | `{INT_OUTPUT_ROOT}/reports/daily/timeline-summary-{date}.txt` |
...
### 5.1.3 Generate Timeline Map (v3.0.0 Enhanced)

> **Purpose**: Enhanced timeline map with Python pre-rendering (원천봉쇄) + LLM narrative analysis.
> "계산은 Python이, 판단은 LLM이" — all deterministic outputs are Python-enforced,
> LLM provides only semantic judgment (trajectory narratives, strategic synthesis).
>
> **실행 순서 주의**: 이 단계는 Step 5.1.4 (통합 통계)보다 먼저 실행된다.
> timeline-summary-{date}.txt를 생산해야 Step 5.1.4가 이를 소비할 수 있기 때문이다.

```yaml
IF EVOLUTION_ENABLED == true AND TIMELINE_MAP_ENABLED == true:
  → Generate enhanced timeline map via sub-orchestrator (with fallback)
ELSE:
...
- `TIMELINE_ORCHESTRATOR` = timeline_map.orchestrator
- `TIMELINE_FALLBACK` = timeline_map.fallback_script

```yaml
IF TIMELINE_ORCHESTRATOR exists:
  → Invoke @timeline-map-orchestrator as Task subagent
  → Pass context: INT_OUTPUT_ROOT, WF1~4_DATA_ROOT, SCAN_DATE,
    CROSS_EVOLUTION_MAP path from Step 5.1.2.4
  → Outputs: timeline-map-{date}.md + timeline-summary-{date}.txt
  → On failure (max 2 retries exhausted within sub-orchestrator):
    → Sub-orchestrator auto-fallbacks to basic generator internally
    → If sub-orchestrator itself fails: run fallback below

ELSE (TIMELINE_ORCHESTRATOR not configured — legacy mode):
  → Run basic generator:
...
- **Note**: `timeline-summary-{date}.txt` is only generated by the enhanced pipeline.
  On success: this file is passed to Step 5.1.4 via `--timeline-summary` argument,
  embedded in report-statistics JSON as INT_TIMELINE_SUMMARY, and fills Section 7.6.
  On fallback: the integrated report's Section 7.6 will use a default message.

### 5.1.4 Compute Integrated Evolution Statistics (Python — v2.3.0)

> **Purpose**: 4개 WF의 evolution-map을 병합하여 EVOLUTION_* 통계 + INT_EVOLUTION_CROSS_TABLE +
> INT_TIMELINE_SUMMARY를 프로그래매틱으로 생성한다. Section 8 placeholders (WF1–WF4 signal
> totals, exec summary)도 이 단계에서 주입된다. 값들은 Step 5.1.5에서 스켈레톤에 주입된다.
>
> **의존성**: Step 5.1.3의 timeline-summary-{date}.txt를 `--timeline-summary`로 수신한다.
...
python3 {TC_STATISTICS_SCRIPT} \
  ...
  --timeline-summary {TIMELINE_SUMMARY_FILE} \
...
- Missing timeline-summary defaults to fallback message for INT_TIMELINE_SUMMARY.
...
EVOLUTION_*, INT_EVOLUTION_CROSS_TABLE, INT_TIMELINE_SUMMARY 플레이스홀더가 사전 주입된다.
